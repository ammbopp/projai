    ...
    public UserList readData() {
        ...
        try {
            ...
            while ( (line = buffer.readLine()) != null ){
                ...
                String roll = data[3].trim();
                String image = data[4].trim();
                String loginTime = data[5].trim();

                // เพิ่มข้อมูลลงใน list
                users.addUser(name, username, password, roll, image, loginTime);
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    public void writeData(UserList data) {
        ...
        try {
            for (User user : data.getUsers()) {
                String line = user.getName() + "," + user.getUsername() + "," + user.getPassword() + "," + user.getRoll() + "," + user.getImage() + "," + user.getLoginTime();
                buffer.append(line);
                buffer.append("\n");
            }
        } catch (IOException e) {
